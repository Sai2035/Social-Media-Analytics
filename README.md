...
### Follower Growth
Measures the change in follower count over time.
...
1. User selects either the Brand or Creator module.  
2. The Flask backend fetches Instagram data using third-party APIs.  
3. Data is processed to compute engagement metrics, growth differences, and sentiment analysis.  
4. Responses are cached in SQLite3 for 12 hours.  
5. The frontend retrieves processed data via REST endpoints.  
6. JavaScript dynamically renders line graphs, bar charts, and pie charts.  
7. Users can export results as CSV for reporting purposes.  
...
3. Ensure the application loads environment variables before starting the Flask server.
